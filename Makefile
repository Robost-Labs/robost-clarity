...
kafka-topics: ## List Kafka topics
	docker exec robost-kafka kafka-topics --bootstrap-server localhost:9092 --list

kafka-create-topic: ## Create llm-traffic-logs topic
	docker exec robost-kafka kafka-topics --bootstrap-server localhost:9092 --create --topic llm-traffic-logs --partitions 3 --replication-factor 1

db-shell: ## Connect to PostgreSQL shell
	docker exec -it robost-postgres psql -U shadow_user -d shadow_ai
