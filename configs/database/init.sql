...
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'robost_user') THEN
        CREATE ROLE robost_user WITH LOGIN PASSWORD 'robost_pass';
    END IF;
END
$$;

-- Grant necessary permissions
GRANT ALL PRIVILEGES ON DATABASE robost_clarity TO robost_user;
GRANT ALL ON SCHEMA public TO robost_user;
...
('kafka_group_id', 'robost-consumer', 'Consumer group ID for Kafka consumption', 'data_sources'),
...
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO robost_user;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO robost_user;
GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA public TO robost_user;-- Multi-Tenancy Migration
