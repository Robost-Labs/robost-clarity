...
class DatabaseService:
        ...
        try:
            with self.get_connection() as conn:
                ...
                if filters.flagged is not None:
                    ...
                    where_params.append(filters.flagged)
                
                if filters.provider:
                    where_conditions.append("LOWER(provider) = LOWER(%s)")
                    where_params.append(filters.provider)
                
                if filters.model:
                    where_conditions.append("LOWER(model) = LOWER(%s)")
                    where_params.append(filters.model)
                
                if filters.src_ip:
                    where_conditions.append("src_ip = %s")
                    where_params.append(filters.src_ip)
                
                if filters.min_risk_score is not None:
                    where_conditions.append("risk_score >= %s")
                    where_params.append(filters.min_risk_score)
                
                if filters.max_risk_score is not None:
                    where_conditions.append("risk_score <= %s")
                    where_params.append(filters.max_risk_score)
                
                if filters.start_date:
                    where_conditions.append("timestamp >= %s")
                    where_params.append(filters.start_date)
                
                if filters.end_date:
                    where_conditions.append("timestamp <= %s")
                    where_params.append(filters.end_date)
                ...
                if admin_view:
                    ...
                    """
                    # Note: SUBSTRING param must be the first parameter in the data query.
                    # Keep it separate from WHERE params to avoid mismatched placeholder ordering.
                    substring_params = [settings.prompt_truncate_length]
                ...
                if admin_view:
                    ...
                    data_params = substring_params + list(where_params)
                data_params.extend([filters.page_size, offset])
    ...
    def _decrypt_record(self, record: Dict, admin_view: bool) -> Dict:
        ...
        try:
            ...
            if admin_view:
                ...
                record['prompt'] = decrypted_prompt if decrypted_prompt is not None else ""
                record['response'] = decrypted_response
                record['headers'] = decrypted_headers
                # Always provide prompt_preview (some response models require it)
                record['prompt_preview'] = record['prompt'][:settings.prompt_truncate_length]
                if len(record['prompt']) > settings.prompt_truncate_length:
    ...
    def get_statistics(self, days: int = 30, organization_id: str = None) -> StatsResponse:
        """Get system statistics, optionally filtered by organization"""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
                ...
                # Top risk IPs - add org filter
                risk_conditions = conditions + ["is_flagged = TRUE"]
                risk_where = " AND ".join(risk_conditions)
                cursor.execute(f"""
                ...
                # Requests by hour (last 24 hours) - add org filter
                hour_conditions = ["timestamp >= NOW() - INTERVAL '24 hours'"]
                if organization_id:
                    hour_conditions.append("organization_id = %s")
                hour_where = " AND ".join(hour_conditions)
                hour_params = [str(organization_id)] if organization_id else []
                cursor.execute(f"""
                    SELECT 
    ...
    def analyze_prompt(self, prompt: str) -> dict:
        """Analyze a prompt against detection rules and return risk assessment"""
        import re
        ...
        try:
            ...
            with self.get_connection() as conn:
                ...
                cursor.execute("""
                    SELECT id, name, pattern, rule_type, severity, points
                ...
                for rule in rules:
                    ...
                    try:
                        ...
                        logger.warning(f"Invalid regex pattern in rule {rule['name']}: {regex_error}")
                        continue
                
                # Calculate risk score (0-100 scale)
                risk_score = min(total_points, 100)
                is_flagged = risk_score >= 50 or any(r['severity'] in ['critical', 'high'] for r in matched_rules)
                
                if matched_rules:
                    flag_reasons = [r['name'] for r in matched_rules[:3]]
            ...
            return {'risk_score': 0, 'is_flagged': False, 'flag_reason': None, 'matched_rules': []}
        ...
        try:
            with self.get_connection() as conn:
                ...
                SELECT 
                    lr.id,
                    CASE 
                        WHEN lr.risk_score >= 80 THEN 'Critical Risk Request'
                        WHEN lr.risk_score >= 60 THEN 'High Risk Request'
                        WHEN lr.risk_score >= 40 THEN 'Medium Risk Request'
                        ELSE 'Suspicious Request'
                    END as title,
                    COALESCE(lr.prompt, 'No prompt available') as description,
                    CASE 
                        WHEN lr.risk_score >= 80 THEN 'critical'
                        WHEN lr.risk_score >= 60 THEN 'high'
                        WHEN lr.risk_score >= 40 THEN 'medium'
                        ELSE 'low'
                    END as severity,
                    'llm_request' as alert_type,
                    CASE 
                        WHEN lr.is_flagged THEN 'new'
                        ELSE 'investigated'
                    END as status,
                    'llm_api' as source_type,
                    lr.id as source_id,
                    lr.id as related_request_id,
                    json_build_object(
                        'model', lr.model,
                ...
                if filters.severity:
                    if filters.severity == 'critical':
                        base_query += " AND lr.risk_score >= 80"
                    elif filters.severity == 'high':
                        base_query += " AND lr.risk_score >= 60 AND lr.risk_score < 80"
                    elif filters.severity == 'medium':
                        base_query += " AND lr.risk_score >= 40 AND lr.risk_score < 60"
                    elif filters.severity == 'low':
                        base_query += " AND lr.risk_score < 40"
                
                if filters.status:
                    if filters.status == 'new':
                        base_query += " AND lr.is_flagged = true"
                    elif filters.status == 'investigated':
                        base_query += " AND lr.is_flagged = false"
                
                # Note: alert_type filter is hardcoded as 'llm_request' in the query
                
                if filters.search:
                    base_query += " AND (lr.prompt ILIKE %s OR lr.model ILIKE %s OR lr.provider ILIKE %s)"
                    search_term = f"%{filters.search}%"
                    params.extend([search_term, search_term, search_term])
                
                if filters.start_date:
                    base_query += " AND lr.created_at >= %s"
                ...
                # Count total records - use a simpler query for counting
                count_query = """
                    SELECT COUNT(*)
                    FROM llm_requests lr
                    WHERE 1=1
                ...
                if filters.severity:
                    if filters.severity == 'critical':
                        count_query += " AND lr.risk_score >= 80"
                    elif filters.severity == 'high':
                        count_query += " AND lr.risk_score >= 60 AND lr.risk_score < 80"
                    elif filters.severity == 'medium':
                        count_query += " AND lr.risk_score >= 40 AND lr.risk_score < 60"
                    elif filters.severity == 'low':
                        count_query += " AND lr.risk_score < 40"
                
                if filters.status:
                    if filters.status == 'new':
                        count_query += " AND lr.is_flagged = true"
                    elif filters.status == 'investigated':
                        count_query += " AND lr.is_flagged = false"
                
                if filters.search:
                    count_query += " AND (lr.prompt ILIKE %s OR lr.model ILIKE %s OR lr.provider ILIKE %s)"
                    search_term = f"%{filters.search}%"
                    count_params.extend([search_term, search_term, search_term])
                
                if filters.start_date:
                    count_query += " AND lr.created_at >= %s"
                ...
                # Add ordering and pagination
                base_query += " ORDER BY lr.created_at DESC"
                base_query += " LIMIT %s OFFSET %s"
                params.extend([filters.page_size, (filters.page - 1) * filters.page_size])
            ...
            with self.get_connection() as conn:
                ...
                cursor.execute("""
                    SELECT 
                        ...
                        COUNT(*) FILTER (WHERE is_flagged = false) as investigated_alerts,
                ...
                return {
                    'total_alerts': row[0],
                    'new_alerts': row[1],
                    'acknowledged_alerts': row[1],  # Using flagged as acknowledged for compatibility
                    'investigated_alerts': row[2],
    ...
    def get_database_stats(self):
        ...
        try:
            with self.get_connection() as conn:
                ...
                cursor.execute("SELECT COUNT(*) FROM llm_requests WHERE is_flagged = true")
    ...
    def get_user_by_username(self, username):
        ...
        try:
            with self.get_connection() as conn:
                ...
                cursor.execute("""
                    SELECT id, username, email, password_hash, first_name, last_name, role, organization_id, is_active, last_login, created_at, updated_at
    ...
    def get_volume_trends(self, time_range: str, date_range: str, provider: Optional[str] = None, model: Optional[str] = None):
        ...
        try:
            with self.get_connection() as conn:
                ...
                # Check if analytics tables have data, fall back to raw table if empty
                cursor.execute("SELECT COUNT(*) as count FROM analytics_hourly")
                analytics_count = cursor.fetchone()['count']
                
                if analytics_count == 0:
                    # Use raw llm_requests table for real-time data
                    return self._get_volume_trends_raw(time_range, date_range, provider, model)
    ...
    def _get_volume_trends_raw(self, time_range: str, date_range: str, provider: Optional[str] = None, model: Optional[str] = None):
        """Get volume trends from raw llm_requests table"""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
                
                # Build the time bucket based on time_range
                if time_range == 'hourly':
                    time_bucket = "DATE_TRUNC('hour', created_at)"
                    date_filter = "created_at >= NOW() - INTERVAL '24 hours'"
                    ...
                    date_filter = "created_at >= NOW() - INTERVAL '4 weeks'"
            ...
            logger.error(f"Failed to get raw volume trends: {e}")
            raise
    ...
    def get_threat_trends(self, time_range: str, date_range: str, provider: Optional[str] = None, model: Optional[str] = None):
        ...
        try:
            with self.get_connection() as conn:
                ...
                if analytics_count == 0:
                    # Use raw llm_requests table for real-time data
                    return self._get_threat_trends_raw(time_range, date_range, provider, model)
    ...
    def _get_threat_trends_raw(self, time_range: str, date_range: str, provider: Optional[str] = None, model: Optional[str] = None):
        """Get threat trends from raw llm_requests table"""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
                ...
                query = f"""
                    SELECT 
                        ...
                        COUNT(*) as total_requests,
                        COUNT(CASE WHEN is_flagged THEN 1 END) as flagged_requests,
                        ROUND(COUNT(CASE WHEN is_flagged THEN 1 END) * 100.0 / NULLIF(COUNT(*), 0), 2) as threat_rate
                    FROM llm_requests
                    WHERE {date_filter} {filter_clause}
                    GROUP BY {time_bucket}
                    ORDER BY {time_bucket}
            ...
            logger.error(f"Failed to get raw threat trends: {e}")
            raise
    ...
    def get_model_usage(self, time_range: str, date_range: str, provider: Optional[str] = None, model: Optional[str] = None):
        ...
        try:
            with self.get_connection() as conn:
                ...
                if analytics_count == 0:
                    # Use raw llm_requests table for real-time data
                    return self._get_model_usage_raw(time_range, date_range, provider, model)
    ...
    def _get_model_usage_raw(self, time_range: str, date_range: str, provider: Optional[str] = None, model: Optional[str] = None):
        """Get model usage from raw llm_requests table"""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
            ...
            logger.error(f"Failed to get raw model usage: {e}")
            raise
    ...
    def get_provider_breakdown(self, time_range: str, date_range: str, provider: Optional[str] = None, model: Optional[str] = None):
        ...
        try:
            with self.get_connection() as conn:
                ...
                if analytics_count == 0:
                    # Use raw llm_requests table for real-time data
                    return self._get_provider_breakdown_raw(time_range, date_range, provider, model)
    ...
    def _get_provider_breakdown_raw(self, time_range: str, date_range: str, provider: Optional[str] = None, model: Optional[str] = None):
        """Get provider breakdown from raw llm_requests table"""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
            ...
            logger.error(f"Failed to get raw provider breakdown: {e}")
            raise
    ...
    # ============================================================
    # Organization and Multi-Tenant Methods
    # ============================================================
            ...
            with self.get_connection() as conn:
                ...
                cursor.execute("""
                    SELECT id, username, email, password_hash, first_name, last_name, 
                           role, organization_id, is_active, last_login, created_at, updated_at
    ...
    def get_organization_by_domain(self, domain: str):
        ...
        try:
            with self.get_connection() as conn:
                ...
                cursor.execute("""
                    SELECT id, name, domain, settings, created_at, updated_at
                    FROM organizations WHERE LOWER(domain) = LOWER(%s)
                """, [domain])
            ...
            logger.error(f"Failed to get organization by domain {domain}: {e}")
            raise

    def create_organization(self, name: str, domain: str, settings: dict = None):
        ...
        try:
            with self.get_connection() as conn:
                ...
                """, [name, domain.lower(), json.dumps(settings or {})])
    ...
    def create_user_with_org(self, email: str, password_hash: str, role: str, 
                             first_name: str, last_name: str, organization_id: UUID):
        ...
        try:
            with self.get_connection() as conn:
                ...
                cursor.execute("""
                    INSERT INTO users (username, email, password_hash, role, first_name, last_name, organization_id)
                    VALUES (%s, %s, %s, %s, %s, %s, %s)
                    RETURNING id, username, email, first_name, last_name, role, organization_id, 
                              is_active, last_login, created_at, updated_at
                """, [username, email.lower(), password_hash, role, first_name, last_name, str(organization_id)])
    ...
    def get_all_organizations(self, page=1, page_size=50, search=None):
        ...
        try:
            with self.get_connection() as conn:
                ...
                if search:
                    where_clause = "WHERE name ILIKE %s OR domain ILIKE %s"
                    search_param = f"%{search}%"
                ...
                data_query = f"""
                    ...
                    {where_clause}
                    GROUP BY o.id, o.name, o.domain, o.settings, o.created_at, o.updated_at
    ...
    def create_llm_request_with_org(self, request_data: dict, organization_id: UUID, user_id: UUID = None):
        """Create an LLM request with organization association (from extension)"""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                ...
                cursor.execute("""
                    INSERT INTO llm_requests (
                        timestamp, src_ip, provider, model, endpoint, method,
                        prompt, response, headers, duration_ms, status_code,
                        risk_score, is_flagged, flag_reason, organization_id, user_id
                    )
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                    RETURNING id
                """, [
                    request_data.get('timestamp', datetime.now()),
                    request_data.get('src_ip', 'extension'),
                    request_data.get('provider', 'unknown'),
                    ...
                    request_data.get('risk_score', 0),
                    request_data.get('is_flagged', False),
                    request_data.get('flag_reason'),
                    str(organization_id),
