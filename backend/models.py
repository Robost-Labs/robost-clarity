...
class UserRole(str, Enum):
    ADMIN = "admin"
    ANALYST = "analyst"  # Read-only console access
    EMPLOYEE = "employee"  # Extension only, NO console access
    READ_ONLY = "read_only"  # Legacy - kept for backward compatibility
...
class OrganizationCreate(BaseModel):
    """Create model for organizations - domain is auto-derived from admin email"""
...
class SignupRequest(BaseModel):
    """Signup request for creating new organization and admin user"""
    email: str = Field(..., description="Business email address (public domains like gmail.com are blocked)")
...
class UserInvite(BaseModel):
    """Model for inviting a new user to an organization"""
    email: str = Field(..., description="Email address of the user to invite")
    first_name: str = Field(..., min_length=1, max_length=50)
    last_name: str = Field(..., min_length=1, max_length=50)
    role: UserRole = Field(..., description="Role for the new user (admin, analyst, or employee)")

class ExtensionAuthRequest(BaseModel):
    """Authentication request from Chrome extension"""
    email: str
    password: str

class ExtensionAuthResponse(BaseModel):
    """Authentication response for Chrome extension"""
    access_token: str
